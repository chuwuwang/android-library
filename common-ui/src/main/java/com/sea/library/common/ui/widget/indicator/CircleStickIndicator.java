package com.sea.library.common.ui.widget.indicator;
...
public class CircleStickIndicator extends View {
    ...
    private void handleStyleable(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.CircleStickIndicator, 0, 0);
        try {
            int defaultPadding = dp2px(5);
            int defaultSelectRadius = dp2px(6);
            int defaultColor = Color.parseColor("#F6DCC7");
            color = typedArray.getColor(R.styleable.CircleStickIndicator_indicator_color, defaultColor);
            radius = typedArray.getDimension(R.styleable.CircleStickIndicator_indicator_radius, defaultPadding);
            padding = typedArray.getDimension(R.styleable.CircleStickIndicator_indicator_padding, defaultPadding);
            selectColor = typedArray.getColor(R.styleable.CircleStickIndicator_indicator_select_color, Color.WHITE);
            selectRadius = typedArray.getDimension(R.styleable.CircleStickIndicator_indicator_select_radius, defaultSelectRadius);
            isStickEnable = typedArray.getBoolean(R.styleable.CircleStickIndicator_indicator_stick_enable, true);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            typedArray.recycle();
